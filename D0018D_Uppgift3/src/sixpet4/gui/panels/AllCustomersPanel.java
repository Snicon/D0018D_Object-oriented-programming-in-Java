...
public class AllCustomersPanel extends BasePanel {
	...
	private BankLogic bankLogic; // Instance of BankLogic, used for data fetching among other things
    ...
    private void populateCustomerTable() {
        ...
        List<String[]> customerData = bankLogic.getAllCustomers(); // Gets all the customer data
    ...
    private void populateAccountTable() {
        ...
        if (personalNumber != null) { // Making sure there is a personal number, otherwise we cannot fetch any accounts.
        	List<String> customerData = bankLogic.getCustomer(personalNumber); // Gets the customer data which includes all the customer accounts
    ...
    private void populateTransactionTable() {
        ...
        if (personalNumber != null && accountNumber != -1) {
        	List<String> transactionData = bankLogic.getTransactions(personalNumber, accountNumber); // Gets all the transaction data
    ...
    /**
     * Helper method for getting the personal number of the currently selected customer in the customerTable (if any selection has been done)
     * @return The personal number as a string if found, null if no customer was selected.
     */
    private String getPersonalNumber() {
    	// Getting the row that has been selected in the table
    	int selectedRow = customerTable.getSelectedRow();
    	
    	// No row selected, indicating no selection was made
    	if (selectedRow == -1) {
    		return null;
    	}
    	
    	// Returning the personal number that probably exists since a valid row has been selected.
    	return (String) customerTable.getValueAt(selectedRow, 2);
    }
    
    /**
     * Helper method for getting the personal number of the currently selected customer in the customerTable (if any selection has been done)
     * @return The account number as an int if found, -1 if no account was selected.
     */
    private int getAccountNumber() {
    	...
    	String accountNumber = (String) accountTable.getValueAt(selectedRow, 0);
    	
    	// Returning the account number that probably exists since a valid row has been selected.
    ...
    private void deleteCustomer() {    	
    	if (personalNumber != null) { // Making sure a customer is selected
    		...
    		List<String> deletedData = bankLogic.deleteCustomer(personalNumber); // Deleting the customer and storing all the data from the method call
    ...
    private void changeCustomerName() {
    	if (personalNumber != null) { // Making sure a customer is selected
    		...
    		List<String> customerData = bankLogic.getCustomer(personalNumber); // Getting the customer data
    		...
    		if (customerData != null) { // Making sure we actually got some customer data
    			...
    			if (newFirstName != null && newLastName != null) { // If the input dialog was cancelled the value will be null, hence why we are checking for it.
    				boolean nameChanged = bankLogic.changeCustomerName(newFirstName, newLastName, personalNumber); // Changing the name and storing how it went in the nameChanged variable
    ...
    private void terminateAccount() {
    	if (personalNumber != null && accountNumber != -1) {
    		String accountData = bankLogic.closeAccount(personalNumber, accountNumber); // Closing the account and storing the data
    ...
    private void deposit() {
    	...
    	if (personalNumber != null && accountNumber != -1) {
        	boolean successful = bankLogic.deposit(personalNumber, accountNumber, amount);
    ...
    private void withdraw() {
    	...
    	if (personalNumber != null && accountNumber != -1) {
        	boolean successful = bankLogic.withdraw(personalNumber, accountNumber, amount);
